...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double factor = 1 - distance * 0.005;
      if(distance>200){
        factor = 0;
      }
      pixel->r = (unsigned char) (factor * (double) pixel->r);
      pixel->g = (unsigned char) (factor * (double) pixel->g);
      pixel->b = (unsigned char) (factor * (double) pixel->b);
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      if(pixel_2->r == 255 && pixel_2->g == 255 && pixel_2->b == 255){
        if(pixel_1->r <= 215){
          pixel_1->r += 40;
        }
        else{
